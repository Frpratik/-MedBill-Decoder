...
function clearReport(){ $('report').hidden=true; $('clear').hidden=true; $('empty').hidden=false; $('rows').replaceChildren(); $('questions').replaceChildren(); $('error').hidden=true; $('status').textContent=''; }
...
function render(report){
  ...
  $('difference').textContent=money(s.amount_above_medicare_benchmark_usd);
  ...
  if(!report.items.length){$('rows').append(node('tr'));const td=node('td','No service rows could be read. Try a clearer itemized bill.');td.colSpan=5;$('rows').lastChild.append(td);}
  for(const item of report.items){
    ...
    const status=node('td');const label=item.flag===true?'Above benchmark':item.availability==='compared'?(item.flag===false?'Not flagged':'Limited comparison'):item.availability==='ocr_review_required'?'Check reading':'No comparison';
